...
const CONCEPTS: Topic[] = [
  ...
  {
    title: 'Objectives',
    body: 'The marked locations — districts, bridges, ports, airfields, depots, anchorages. Occupy one to take control. An objective is only paid for once your opponent has had their reply and you are STILL holding it, so ground you take and immediately lose scores nothing: objectives have to be held, not merely touched. Objectives are not all worth the same. The ones on the axis between the two deployment areas — the towns, the trunk river crossings, the commanding hills — are worth two to three times a rear-area objective, so a force that stays home cannot reach the threshold in the time available. IMPORTANT: the match ends the moment either side reaches the VP threshold (280 by default) — it does not necessarily run to the round limit, and most matches end well before it. The round counter in the HUD shows the ceiling, not a promise the match runs that long; watch the VP meter next to it for how close the match is to actually ending. Only a match that never reaches the threshold runs the full round limit (24 by default), at which point the higher score wins.',
  },
  {
